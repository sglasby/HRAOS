...
using System.Text;
    ...
    public class DenseGrid : IGridIterable
    {
        ...
        public DenseGrid(int ww, int hh, int fill)
        {
            ...
            if (fill == 0) return;  // Already initialized with zero, no point looping for a no-op
            this.fill(fill);
        } // DenseGrid()
        ...
        {
            if (from_A == null) return null;
            if (to_B   == null) return null;

            // The from_x,y are relative to the origin of from_A.
            if (from_x  < from_A.min_x()) return null; // At least somewhat off-edge to the left
            if (from_x  > from_A.max_x()) return null; // Entirely to the right
            if (from_y  < from_A.min_y()) return null; // At least somewhat off-edge above
            if (from_y  > from_A.max_y()) return null; // Entirely below

            // The to_x,y are relative to the origin of to_B.
            if (to_x + blit_width  < to_B.min_x()) return null; // Entirely to the left
            if (to_x               > to_B.max_x()) return null; // Entirely to the right
            if (to_y + blit_height < to_B.min_y()) return null; // Entirely above
            if (to_y               > to_B.max_y()) return null; // Entirely below
