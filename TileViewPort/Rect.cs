...
namespace WinForms_display_bitmap
{
    public class Rect    // Hmmm...fulfills part of IGridIterable, shows the need for more interfaces for the spectrum of geometry functionality...
    {
        ...
        public bool intersects(Rect rr)
        {
            ...
            if (rr.max_x() < this.x      ) return false;
            if (rr.x       > this.max_x()) return false;
            if (rr.max_y() < this.y      ) return false;
            if (rr.y       > this.max_y()) return false;
            return true;
        } // intersects(rr)
        ...
        {
            if (!this.intersects(rr)) return null;
